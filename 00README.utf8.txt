    ...
    (Created: 2015-10-16 +0900, Time-stamp: <2026-05-24T15:36:31Z>)
...
インストール方法の大枠は次の通りである。
  ...
  * SKK-JISYO.L や tankanji-2026.txt や emoji-skk-dic.txt などの変換用
    辞書の入手＆インデックス・データベースファイルの作成。
...
ピーしておいたほうがいいだろう。
  ...
  * SKK-JISYO.L や tankanji-2026.txt などの変換用辞書の入手＆インデッ
    クス・データベースファイルの作成。

単漢字変換辞書は、例えば↓から tankanji-2026.txt が持って来れる。

   《GitHub - JRF-2018/jrf_tankanji-2026: JRF が(同人的に)作成した単漢字辞書》  
...
手に入れた tankanji-2026.txt と SKK-JISYO.L と emoji-skk-dic.txt と
...
…とする。オプションの -e は euc-jisx0213 を使うという意味で、「風」辞
書を使う場合など単漢字辞書 が sjis の場合は -s を指定すれば良い。-u と
すれば、utf8 になる。

これで、SKK-JISYO.L.sdb.pag , SKK-JISYO.L.sdb.dir ,
tankanji-2026.txt.sdb.pag , tankanji-2026.txt.sdb.dir ができるはずである。
...
で、-u は utf-8、この二番目の引き数の位置になければならない。もし「風」
辞書を使う場合など単漢字辞書が sjis であれば、-s を指定する。
...
■ S-SPC による alt-convert 拡張 (新機能: 2026-05-19)

最初にシフトキーを押して「Nihon」と入力して変換するのも「nohon:」と入
力して変換するのも同じ感じになるのであるが、「風」系の哲学として、入力
したあとのキーで選びたいということがある。だから、「nihon」の後 S-SPC
で nihon: SPC と入力したのと同じように変換したほうが便利だということで
2026年5月にデフォルトをそう変えた。

その替わりに、これまでの S-SPC に割り当たっていた、入力中にどうしても
スペースを入れたいというときのキーは、C-SPC に移動した。

ただ、もし、ラテン語圏などで S-SPC をこれまで頻繁に使っている人がいる
と、それは迷惑だろう。その場合は以下のように設定するとよい。

<source>
(setq quail-naggy-shift-space-alt-convert-p nil)
</source>
