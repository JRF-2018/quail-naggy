...
;; S-SPC による alt-convert 拡張。

(defvar quail-naggy-shift-space-alt-convert-p t
  "非 nil なら、[?\\S- ] による alt-convertを有効化。")
...
(defun quail-naggy-update-conversion-keymap ()
  "現在の `quail-naggy-shift-space-alt-convert-p' の値に応じてキーバインドを更新する。"
  (when (and (fboundp 'quail-name)
             (string= (quail-name) "japanese-naggy"))
    (let ((map (quail-conversion-keymap)))
      (if quail-naggy-shift-space-alt-convert-p
          (progn
            (define-key map [?\S- ] 'quail-naggy-alt-convert)
            (define-key map [?\C- ] 'quail-naggy-insert-space))
        ;; オフのときは初期値（デフォルト）に戻す
        (define-key map [?\S- ] 'quail-naggy-insert-space)))))

;; インプットメソッド起動時に自動適用するフック
(add-hook 'input-method-activate-hook #'quail-naggy-update-conversion-keymap)
