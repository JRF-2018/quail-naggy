...
#require 5.008;
{ package main;
  my $TS = 'Time-stamp: <2016-02-09T04:03:06Z>';
  $TS =~ s/Time-stamp\:\s+<(.*)>/$1/;
  my $AUTHOR = "JRF (http://jrf.cocolog-nifty.com/)";
  our $VERSION = "0.02; make_skk_dic_dbm.pl; last modified at $TS; by $AUTHOR";
  our $DEBUG = 1;
}
...
MAIN:
{
  ...
  while (1) {
    ...
    $s =~ s/\s+$//s;
    next if $s eq "";
    my ($yomi, $ks) = split(/\s+/, $s, 2);
    my $key = Unicode::Japanese->new($yomi)->hira2kata->z2h->sjis;
    my $size = $tail - $pos;
    $dic{$key} = pack("Vv", $pos, $size);
#    print "$yomi $pos $size\n";
  }
  untie %dic;
  close($ih);
...
make_skk_dic_db.pl - prepares an SDBM_File of an SKK dictionary.

=head1	SYNOPSIS

B<make_skk_dic_db.pl> [-e|-s|-u] TEXT_FILE
